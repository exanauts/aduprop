...
void fdT_driver(double* xic, size_t dim, System* sys, double h, double**** T) {
  ...
  for(size_t i = 0 ; i < dim ; ++i) {
    ...
    xpert1[i] += pert/2.0;
    xpert2[i] -= pert/2.0;
    t2s_t1s_driver(xpert1, dim, sys, h, J, Hpert1);
    t2s_t1s_driver(xpert2, dim, sys, h, J, Hpert2);
    for(size_t j = 0 ; j < dim ; ++j) {
      for(size_t k = 0 ; k < dim ; ++k) {
  ...
  for(size_t i = 0; i < dim; ++i) {
    for(size_t j = 0; j < dim ; ++j) {
      for(size_t k = 0; k < dim ; ++k) {
        ...
        t3s_t2s_t1s_integrate(axic, dim, sys, h);
        for(size_t l = 0; l < dim ; ++l) {
...
int main(int nargs, char** args) {
  ...
  for(size_t i = 0; i < dim; ++i) x[i] = xold[i];
  t3s_t2s_t1s_driver(x, dim, &sys, h, J, H, T);
  cout << "J" << endl;
  for(size_t i = 0; i < dim; ++i) {
    for(size_t j = 0; j < dim; ++j) {
      cout << J[i][j] << " ";
